...
class ModelArguments:
    ...
    # ✅ CORRECCIÓN LO-RA: Añadir los argumentos de LoRA a ModelArguments
    lora_r: int = field(
        default=8,
...
# ... (resto de DataSeq2SeqTrainingArguments) ...
...
def main():
    ...
    def prepare_audio_features(batch):
        try:
            ...
            # Verificar que el audio no esté silencioso
            ...
            logger.error(f"Error procesando el archivo de audio {batch.get(target_audio_column_name, 'N/A')}: {str(e)}")
            batch["labels"] = None
            return batch
        ...
        try:
            ...
            logger.error(f"Error procesando el texto para {batch.get(target_audio_column_name, 'N/A')}: {e}")
            batch["input_ids"] = instance_prompt_tokenized
    ...
    if model_args.use_lora:
        ...
        # ✅ CORRECCIÓN LO-RA: Usar directamente los argumentos de ModelArguments
        ...
        config = LoraConfig(
            r=lora_r,  # 🔥 Valor tomado de model_args.lora_r
            lora_alpha=lora_alpha,  # 🔥 Valor tomado de model_args.lora_alpha
    ...
    if training_args.do_train:
        ...
        # ✅ GUARDAR EXPLÍCITAMENTE LOS ADAPTADORES LoRA
        if model_args.use_lora:
            # Guardamos SOLO los pesos LoRA (adapter_model.safetensors + adapter_config.json)
            model.save_pretrained(training_args.output_dir)
            logger.info(f"✅ Adaptadores LoRA guardados en {training_args.output_dir}")
        else:
            trainer.save_model()

        # Guardar también el procesador y la configuración (siempre)
        if is_main_process(training_args.local_rank):
            processor.save_pretrained(training_args.output_dir)
            config.save_pretrained(training_args.output_dir)
